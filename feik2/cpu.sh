#!/bin/sh
rrdtool create cpu.rrd -b now-100d -s 1d \
DS:cpu:GAUGE:1d:U:U \
...
rrdtool fetch cpu.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update cpu.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r; system(cmd);}'

rrdtool graph cpu.png --start now-100d --end=now \
DEF:obs=cpu.rrd:cpu:AVERAGE \
DEF:max=cpu.rrd:cpu:MAX \
DEF:min=cpu.rrd:cpu:MIN \
DEF:pred=cpu.rrd:cpu:HWPREDICT \
DEF:dev=cpu.rrd:cpu:DEVPREDICT \
DEF:fail=cpu.rrd:cpu:FAILURES \
...
LINE2:scpucaledobs#039be5:"Average bits out" \
...
#rrdtool graph test.gif --start now-100d --end=now DEF:test=test.rrd:cpu:AVERAGE LINE2:test#FF0000:test 
