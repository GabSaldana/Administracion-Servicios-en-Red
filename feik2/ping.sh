#!/bin/sh
rrdtool create ping.rrd -b now-100d -s 1d \
DS:ping:GAUGE:1d:U:U \
...
rrdtool fetch ping.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update ping.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r; system(cmd);}'

rrdtool graph ping.png --start now-100d --end=now \
DEF:obs=ping.rrd:ping:AVERAGE \
DEF:max=ping.rrd:ping:MAX \
DEF:min=ping.rrd:ping:MIN \
DEF:pred=ping.rrd:ping:HWPREDICT \
DEF:dev=ping.rrd:ping:DEVPREDICT \
DEF:fail=ping.rrd:ping:FAILURES \
...
LINE2:spingcaledobs#039be5:"Average bits out" \
...
#rrdtool graph test.gif --start now-100d --end=now DEF:test=test.rrd:ping:AVERAGE LINE2:test#FF0000:test 
