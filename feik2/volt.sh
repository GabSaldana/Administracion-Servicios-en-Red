#!/bin/sh
rrdtool create volt.rrd -b now-100d -s 1d \
DS:volt:GAUGE:1d:U:U \
...
rrdtool fetch volt.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update volt.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r; system(cmd);}'

rrdtool graph volt.png --start now-100d --end=now \
DEF:obs=volt.rrd:volt:AVERAGE \
DEF:max=volt.rrd:volt:MAX \
DEF:min=volt.rrd:volt:MIN \
DEF:pred=volt.rrd:volt:HWPREDICT \
DEF:dev=volt.rrd:volt:DEVPREDICT \
DEF:fail=volt.rrd:volt:FAILURES \
...
LINE2:svoltcaledobs#039be5:"Average bits out" \
...
#rrdtool graph test.gif --start now-100d --end=now DEF:test=test.rrd:volt:AVERAGE LINE2:test#FF0000:test 
