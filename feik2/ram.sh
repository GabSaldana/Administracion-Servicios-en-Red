#!/bin/sh
rrdtool create ram.rrd -b now-100d -s 1d \
DS:ram:GAUGE:1d:U:U \
...
rrdtool fetch ram.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update ram.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r; system(cmd);}'

rrdtool graph ram.png --start now-100d --end=now \
DEF:obs=ram.rrd:ram:AVERAGE \
DEF:max=ram.rrd:ram:MAX \
DEF:min=ram.rrd:ram:MIN \
DEF:pred=ram.rrd:ram:HWPREDICT \
DEF:dev=ram.rrd:ram:DEVPREDICT \
DEF:fail=ram.rrd:ram:FAILURES \
...
LINE2:sramcaledobs#039be5:"Average bits out" \
...
#rrdtool graph test.gif --start now-100d --end=now DEF:test=test.rrd:ram:AVERAGE LINE2:test#FF0000:test 
