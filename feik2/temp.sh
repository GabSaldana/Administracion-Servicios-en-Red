#!/bin/sh
rrdtool create temp.rrd -b now-100d -s 1d \
DS:temp:GAUGE:1d:U:U \
...
rrdtool fetch temp.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update temp.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r; system(cmd);}'

rrdtool graph temp.png --start now-100d --end=now \
DEF:obs=temp.rrd:temp:AVERAGE \
DEF:max=temp.rrd:temp:MAX \
DEF:min=temp.rrd:temp:MIN \
DEF:pred=temp.rrd:temp:HWPREDICT \
DEF:dev=temp.rrd:temp:DEVPREDICT \
DEF:fail=temp.rrd:temp:FAILURES \
...
LINE2:stempcaledobs#039be5:"Average bits out" \
...
#rrdtool graph test.gif --start now-100d --end=now DEF:test=test.rrd:temp:AVERAGE LINE2:test#FF0000:test 
