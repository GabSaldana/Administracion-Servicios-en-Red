#!/bin/sh
rrdtool create prueba.rrd -b now-100d -s 1d \
DS:msgs:GAUGE:1d:U:U \
...
rrdtool fetch prueba.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update prueba.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r; system(cmd);}'

rrdtool graph prueba.png --start now-100d --end=now \
DEF:obs=prueba.rrd:msgs:AVERAGE \
DEF:max=prueba.rrd:msgs:MAX \
DEF:min=prueba.rrd:msgs:MIN \
DEF:pred=prueba.rrd:msgs:HWPREDICT \
DEF:dev=prueba.rrd:msgs:DEVPREDICT \
DEF:fail=prueba.rrd:msgs:FAILURES \
