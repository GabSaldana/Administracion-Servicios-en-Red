#!/bin/sh
rrdtool create py.rrd -b now-100d -s 1d \
...
rrdtool fetch py.rrd AVERAGE -s -100d | awk '/:/ {cmd="rrdtool update py.rrd " $1 q; print cmd; s=.25*q;v=v+1; u=v/100; t=v%8; r=rand();q=3+s+v+2*u+3*(t-4)*(t-4)+10*r;if(v>70 && v< 80)q=0; system(cmd);}'

rrdtool graph py.png --start now-100d --end=now \
DEF:obs=py.rrd:msgs:AVERAGE \
DEF:max=py.rrd:msgs:MAX \
DEF:min=py.rrd:msgs:MIN \
DEF:pred=py.rrd:msgs:HWPREDICT \
DEF:dev=py.rrd:msgs:DEVPREDICT \
DEF:fail=py.rrd:msgs:FAILURES \
